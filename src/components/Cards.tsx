...
const CardTeaser = ({ title, description, image, link }: Card) => {
    return (
      <motion.div 
        className="group relative min-h-[80vh] overflow-hidden"
      ...
      >
        ...
        />
          ...
          {/* Black Mask that slides up */}
          {/* <div className="absolute inset-0 bg-black transition-transform duration-700 group-hover:translate-y-[-100%] z-10" /> */}
        ...
        <div className="relative h-full p-12 flex flex-col justify-between">
          <div className="h-full p-12 flex justify-center flex-col">
            <div className="transform transition-all duration-700 translate-y-[0%] group-hover:-translate-y-1/2">
              <h3 className="text-4xl font-bold text-white mb-4 group-hover:text-black">{title}</h3>
              <motion.p 
                className="text-xl max-h-0 opacity-0 overflow-hidden transition-all duration-500 group-hover:opacity-100 group-hover:text-black group-hover:max-h-[500px]"
              >
                {description}
              </motion.p>
              <Link 
              ...
              <div className="w-full h-full fhd:h-[350px] 2k:h-[450px] absolute left-0 bottom-0 opacity-0 translate-y-[30%] group-hover:translate-y-[0%] group-hover:opacity-100 transition-all duration-500">
               <img className="w-full h-full object-cover" src="/seo.jpg" alt="" />
              </div>
          </div>
        </div>
      </motion.div>
